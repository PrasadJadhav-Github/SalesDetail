package com.gadre.salesdetail.Database
...
class DBHelper(context: Context?) : SQLiteOpenHelper(context, "DbSalesRecords", null, 1) {
    ...
    fun selectCity(): List<TCity>? {  //ArrayList is used to collect and return the list of TCity objects
        ...
        val cursor: Cursor? = db.rawQuery("SELECT * FROM ${ColumnHelper.TABLE_NAME_CITY}", arrayOf())
    ...
    fun selectpersonname(): List<TSalesPerson>? {
        ...
        val curser: Cursor? = db.rawQuery("SELECT * FROM ${ColumnHelper.TABLE_NAME_SELLS_PERSPN}", arrayOf())
        var globalarrperson: ArrayList<TSalesPerson> = ArrayList()

        if (curser != null) {
            if (curser.moveToFirst()) {
                while (curser.moveToNext()) {
                    val name = curser.getString(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_PERSON_NAME))
                    val id =curser.getInt(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_PERSON_ID))
    ...
    fun selectSalesDetails(month: String,year: String):ArrayList<SalesInfo>?{
        val db= this.readableDatabase
        val curser=db.rawQuery("SELECT * FROM ${ColumnHelper.TABLE_SALES} WHERE ${ColumnHelper.TSALES_MONTH}='"+month.lowercase()+"' AND ${ColumnHelper.TSALES_YEAR}='"+year+"'", arrayOf())
        val globalArraySales:ArrayList<SalesInfo> =ArrayList()
        if (curser!=null){
            if (curser.moveToFirst()){
                while (curser.moveToNext()){
                    val month = curser.getString(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_MONTH))
                    val year = curser.getString(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_YEAR))
                    val personFk = curser.getInt(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_PERSON_FK))
                    val cityFk = curser.getInt(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_CITY_FK))
                    val sales = curser.getDouble(curser.getColumnIndexOrThrow(ColumnHelper.TSALES_SELLS))

                    val city = selectCityFromId(cityFk)
                    val person=selectPersonFromId(personFk)
                    val salesInfo = SalesInfo(month = month, year = year,sales=sales, cityName = city.cityname, salesPersonName = person.personname)
    ...
    fun selectCityFromId(id: Int):TCity{
        val db =this.readableDatabase
       val cursor= db.rawQuery("SELECT * FROM ${ColumnHelper.TABLE_NAME_CITY} WHERE  ${ColumnHelper.TCITY_ID}=$id ", arrayOf() )
    ...
    fun selectPersonFromId(id: Int):TSalesPerson{
        val db=this.readableDatabase
        val cursor =db.rawQuery("SELECT * FROM ${ColumnHelper.TABLE_NAME_SELLS_PERSPN} WHERE ${ColumnHelper.TSALES_PERSON_ID}=$id",arrayOf())
        var tPersonName:TSalesPerson=TSalesPerson(" ")
        if (cursor.moveToFirst()){
            val personname=cursor.getString(cursor.getColumnIndexOrThrow(ColumnHelper.TSALES_PERSON_NAME))
